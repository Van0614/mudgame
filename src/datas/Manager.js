...
const eventChooser = (x, y, randomKey) => {
  // Todo: randomkey는 player.randomKey
  // TODO: 맵은 0~10으로 가정하고, 5,5에 가까워질수록 강한 보상과 맵이 나온다.
  const messages = [
    // 가장자리부터 중심부까지 순서대로 메세지 출력
    "평화롭다.",
    "약간 긴장된다.",
    "스산하다.",
    "기운이 어둡다.",
  ...
  const distanceMonster = JSON.parse(
    fs.readFileSync(__dirname + "/monsters.json")
  );
  ...
  if (distanceFromCenter === 0) {
    ...
    response.message = "야생의" + response.monsterName + "(이)가 싸움을 걸어왔다"
  } else {
    if (placeSeed < 5) {
      response.event = "none";
